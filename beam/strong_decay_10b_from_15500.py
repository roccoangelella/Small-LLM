#!/usr/bin/env python3
"""Resume 100M/10B from exact step 15,500 with stronger decay throughout.

Usage from repository root:

    python beam/strong_decay_10b_from_15500.py --dry-run
    python beam/strong_decay_10b_from_15500.py --gpu RTX4090
...
2. inverse-square-root decay from the 1e-4 settled anchor to 9.6B targets;
...
RUN_ID = "100m-10b-strong-decay-from-step15500"
...
CONTRACT_PATH = RUN_DIR / "strong_decay_10b_contract.json"
...
def _expected_lr(tokens: int) -> float:
    ...
    base_lr = SETTLE_LR * math.sqrt(SETTLE_END_TOKENS / tokens)
...
def _contract() -> dict[str, object]:
    return {
        "version": 1,
        "kind": "step15500_strong_decay_settle_invsqrt_linear_cooldown",
...
@function(
    name="small-llm-strong-decay-10b-15500-prepare",
...
def prepare_remote(source_commit: str) -> dict[str, object]:
    ...
    dataset = base.CACHE_ROOT / "strong-decay-10b" / RUN_ID / f"from-{required_block:08d}"
    ...
    if latest_step == 0:
        ...
        raise RuntimeError("resumed continuation is missing strong_decay_10b_contract.json")
    elif json.loads(CONTRACT_PATH.read_text(encoding="utf-8")) != _contract():
        raise RuntimeError("strong-decay contract drifted between segments")
...
@function(
    name="small-llm-strong-decay-10b-15500-visibility",
    image=base.CPU_IMAGE,
    cpu=2,
    memory="8Gi",
    timeout=180,
...
def _train_impl(
    ...
    _replace_option(
        command,
        "--wandb-run-name",
        "100M/10B strong-decay continuation from step 15500",
    )
...
@function(
    name="small-llm-strong-decay-10b-15500-rtx5090",
    gpu="RTX5090",
...
@function(
    name="small-llm-strong-decay-10b-15500-rtx4090",
    gpu="RTX4090",
...
@function(
    name="small-llm-strong-decay-10b-15500-a10g",
    gpu="A10G",
    image=base.LEGACY_SERVERLESS_IMAGE,
    **base._GPU_FUNCTION_KWARGS,
)
...
def main() -> int:
    ...
    payload = {
        "action": "step15500_strong_decay_10b_continuation",
        "runtime": "beam/strong_decay_10b_from_15500.py",
        "gpu": args.gpu,
        "source_run_id": SOURCE_RUN_ID,
        "source_checkpoint_id": SOURCE_CHECKPOINT_ID,
        "run_id": RUN_ID,
        ...
        "final_step": FINAL_STEP,
        "final_targets": TOTAL_TARGETS,
        "schedule": "300M cosine to 1e-4 + inverse-sqrt base + 400M linear to 1.5e-5",
        "source_commit": source_commit,
    }
    ...
    base._stage("strong_decay_10b_prepare_start", checkpoint=SOURCE_CHECKPOINT_ID)
    prepared = base._require_remote_mapping(
        prepare_remote.remote(source_commit),
        label="strong-decay 10B prepare",
    )
    base._stage("strong_decay_10b_prepare_complete", **prepared)
    ...
    base._stage("strong_decay_10b_visibility_start", required_block=required_block)
    visible = base._require_remote_mapping(
        verify_stage_remote.remote(dataset_dir, required_block),
        label="strong-decay dataset visibility",
    )
    base._stage("strong_decay_10b_visibility_complete", **visible)

    base._stage(
        "strong_decay_10b_dispatch",
        gpu=args.gpu,
    ...
    result = base._require_remote_mapping(
        ...
        label="strong-decay 10B training",
    )
