#!/usr/bin/env python3
"""Temporary Beam counterfactual: 100M/10B step 15,500 + controlled cooldown.

Usage from a clean repository root:

    python beam/decay_probe_15500.py --dry-run
    python beam/decay_probe_15500.py --gpu RTX4090

The originally selected step-12,500 checkpoint is no longer retained. This
probe therefore forks the earliest recoverable exact Beam checkpoint,
``step-00015500``. It preserves model, optimizer, scaler, RNG, and data cursor,
and changes only the token LR schedule.

The controlled cooldown starts from the production peak LR (3e-4) at the fork
point and cosine-decays to 3e-5 over the nearest whole-block span to 400M target
tokens: 3,052 optimizer updates / 400,031,744 targets. The probe therefore ends
at global step 18,552 while continuing the exact 10B corpus order.
"""
...
PROBE_RUN_ID = "100m-10b-decay-probe-step15500"
...
REQUESTED_COOLDOWN_TOKENS = 400_000_000
SOURCE_EXPECTED_TOKENS = SOURCE_STEP * TARGETS_PER_FULL_BLOCK
PROBE_ADDITIONAL_STEPS = math.ceil(REQUESTED_COOLDOWN_TOKENS / TARGETS_PER_FULL_BLOCK)
COOLDOWN_TOKENS = PROBE_ADDITIONAL_STEPS * TARGETS_PER_FULL_BLOCK
...
def _expected_lr(tokens: int) -> float:
    """Controlled cosine cooldown, expressed in absolute committed tokens."""

    if tokens <= SOURCE_EXPECTED_TOKENS:
        return PEAK_LR
    progress = min(
        1.0,
        max(0.0, (tokens - SOURCE_EXPECTED_TOKENS) / COOLDOWN_TOKENS),
    )
...
def _probe_contract() -> dict[str, object]:
    return {
        "version": 2,
        "kind": "temporary_step15500_controlled_cosine_cooldown_probe",
        ...
        "warmup_tokens": 0,
        "stable_tokens": SOURCE_EXPECTED_TOKENS,
        "requested_cooldown_tokens": REQUESTED_COOLDOWN_TOKENS,
        "decay_tokens": COOLDOWN_TOKENS,
        "additional_steps": PROBE_ADDITIONAL_STEPS,
        "final_step": PROBE_FINAL_STEP,
        "additional_full_block_targets": COOLDOWN_TOKENS,
...
def _fork_source_checkpoint(*, dataset: Path) -> None:
    """Fork exact step 15,500 and patch only trainer/scheduler configuration."""
    ...
    try:
        ...
        # WSD's stable_end is absolute in committed-token space. Setting
        # stable_tokens to the source token count and warmup to zero makes the
        # exact fork point the start of the new cooldown: LR(source)=3e-4, then
        # the next successful optimizer update begins cosine decay.
...
@function(
    name="small-llm-decay-probe-15500-prepare",
...
def prepare_probe_remote(source_commit: str) -> dict[str, object]:
    ...
    if latest_step == 0:
        ...
        if not source.is_dir():
            raise RuntimeError(
                ...
                "the probe will not substitute another checkpoint"
            )
        verify_local_manifest(source)
    if latest_step == PROBE_FINAL_STEP:
        return {
    ...
    if latest_step == 0:
        ...
        if latest_id != SOURCE_CHECKPOINT_ID or latest_step != SOURCE_STEP:
            raise RuntimeError("probe fork did not install the exact step-15500 checkpoint")
...
@function(
    name="small-llm-decay-probe-15500-visibility",
    image=base.CPU_IMAGE,
    cpu=2,
    memory="8Gi",
    timeout=180,
...
def _train_probe_impl(
    ...
    _replace_option(command, "--wandb-run-name", "100M/10B step-15500 controlled cooldown probe")
...
@function(
    name="small-llm-decay-probe-15500-rtx5090",
    gpu="RTX5090",
...
@function(
    name="small-llm-decay-probe-15500-rtx4090",
    gpu="RTX4090",
...
@function(
    name="small-llm-decay-probe-15500-a10g",
    gpu="A10G",
    image=base.LEGACY_SERVERLESS_IMAGE,
    **base._GPU_FUNCTION_KWARGS,
)
...
def main() -> int:
    ...
    payload = {
        "action": "step15500_controlled_cosine_cooldown_probe",
        "runtime": "beam/decay_probe_15500.py",
        "gpu": args.gpu,
        "source_run_id": SOURCE_RUN_ID,
        "source_checkpoint_id": SOURCE_CHECKPOINT_ID,
        "probe_run_id": PROBE_RUN_ID,
        ...
        "lr_final": PEAK_LR * MINIMUM_LR_RATIO,
        "requested_cooldown_tokens": REQUESTED_COOLDOWN_TOKENS,
        "actual_cooldown_tokens": COOLDOWN_TOKENS,
        "additional_steps": PROBE_ADDITIONAL_STEPS,
        "final_step": PROBE_FINAL_STEP,
        "schedule": "400M block-aligned controlled cosine cooldown",
        "source_commit": source_commit,
    }
