"""Fixed finite-dataset entry point for the 20M-model/2B-token scaling run."""
...
def qualification_arguments(argv: Sequence[str]) -> list[str]:
    """Append the fixed 2B profile while retaining safe producer tuning."""
    ...
    if conflicts:
        raise SystemExit(
            "the 20M-model/2B-token dataset fixes these arguments: "
            + ", ".join(conflicts)
        )
    return [
        *argv,
