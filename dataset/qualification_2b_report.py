"""Derive the exact one-pass plan for the 20M-model/2B-token scaling run."""
...
from dataset.qualification_2b import (
...
def main(argv: Sequence[str] | None = None) -> int:
    return run_cli(
        ...
        profile=PROFILE,
        description=(
            "Derive the exact 20M-model/2B-token trainer plan from its "
            "verified manifest."
        ),
    )
