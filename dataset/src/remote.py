...
durable mirror after verified read-back. Google Drive is intentionally not an
active backend anymore; legacy ``drive_manifest.json`` checkpoint metadata is
still readable elsewhere for historical-run compatibility.
"""
...
class InMemoryShardStore:
    """Byte-accurate provider-neutral shard-store fake for deterministic tests."""
    ...
    def list_manifest_entries(self, run_id: str) -> list[dict[str, object]]:
        ...
        return [
            ...
            for (rid, name), file_id in self.names.items()
            if rid == run_id
        ]
...
def mirror_finalized_shard(
    ...
    *,
    ...
    """Upload and verify one immutable shard.

    The output retains historical ``drive_*`` metadata keys because existing
    checkpoint and dataset artifacts use that schema. They are provider-neutral
    compatibility fields now; no Google Drive implementation is involved.
    """
    ...
    compatibility_checksums: dict[str, object] = {
        "sha256": verified["sha256"],
        "sha256_source": verified.get("sha256_source", "verified remote read-back"),
    }
    return {
        **dict(entry),
        "run_id": run_id,
        ...
        "drive_checksums": compatibility_checksums,
        "remote_durable": True,
        "verification_timestamp": verified["verified_at"],
        "configuration_hash": config_hash,
        "schema_hash": schema_hash,
    }


def write_drive_manifest(
    path: Path,
    *,
    run_id: str,
    ...
    """Write the legacy durability-manifest schema used by existing checkpoints."""
...
class HuggingFaceCheckpointStore:
    ...
    def _upload_file(self, path: Path, path_in_repo: str) -> str:
        ...
        if not remote_path.is_file():
            raise RuntimeError(
                f"Hugging Face download did not return a regular file: {path_in_repo}"
            )
...
class TwoPhaseCheckpointPublisher:
    """Publish a verified last snapshot before moving latest.json."""
    ...
    def _verify_upload_response(
        ...
        prefix: str,
        local_dir: Path,
        uploaded: Mapping[str, str] | object,
    ) -> None:
        ...
        if missing or unexpected:
            ...
            raise RuntimeError(
                "remote checkpoint upload file set mismatch (" + ", ".join(details) + ")"
            )
        mismatched = sorted(
            key
    ...
    def publish(
        ...
        *,
        ...
        if drive_manifest.get("version") != 1 or drive_manifest.get("run_id") != run_id:
            raise RuntimeError("durability manifest version or run_id does not match this publication")
        ...
        if not isinstance(shards, list) or any(
            ...
            raise RuntimeError(
                "cannot publish checkpoint before every referenced dataset shard is verified"
            )
    ...
    def cleanup_history(
        ...
        *,
        ...
        if not destructive or not remote_verified:
            raise RuntimeError(
                "history cleanup is irreversible, disabled by default, and requires verified remote state"
            )
        raise NotImplementedError(
            "destructive cleanup must be invoked by an explicit deployment command"
        )
