...
What SVD does (brief linear algebra review), is expressing a matrix $M$ as the product of the matrices $M = U \Sigma V^\top$, where $U$ and $V$ describe a rotation, and $\Sigma$ represents a shrink/stretch. The definition of SVD I give myself is that, provided a transformation matrix $A$, we use SVD to split the overall transformation in "two rotations and one shrink/stretch".

So, in SVD, $U, V$ represent which directions matter the most in our transformation, and $\Sigma$ describes **how strongly each direction is stretched/shrunk**.

So, after taking the SVD of the Momentum matrix, **Muon replaces $\Sigma$ values with approximately the identity matrix**, therefore our SVD becomes roughly $U V^\top$. Therefore, the directions that exist in each matrix update obtain roughly the same importance.
...
Then, at each step, SOAP rotates the moving avg of the gradients in this coordinate system: $G' = Q_L^\top G Q_R$. This rotated gradient matrix features directions aligned with coordinate axes (won't go deeper on the meaning of this, I'm tired lol). Done that, we apply Adam on the rotated space, using the classic two moments update, and finally rotating back into the original matrix coordinates by $A = Q_L A' Q_R^\top$.

Finally, $W_{\text{new}} = W - \eta A$.