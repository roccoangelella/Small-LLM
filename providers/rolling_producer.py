"""Shared Beam/Modal orchestration for incremental dataset production."""
...
def _dataset_bucket_id(*, base_runtime: Any) -> str:
    import os
...
def produce_incremental_dataset(
    *,
    ...
    bucket_id = _dataset_bucket_id(base_runtime=base_runtime)
    ...
    print(
        json.dumps({f"{provider_name}_cpu_dataset_producer": result}, sort_keys=True),
        flush=True,
    )
    return result
