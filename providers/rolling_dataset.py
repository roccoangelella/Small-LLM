"""Shared Beam/Modal orchestration for the HF rolling dataset transport.

Provider launchers inject their runtime and profile resolver. Dataset transport
semantics stay in :mod:`dataset`; provider SDK imports stay out of this module.
"""
...
def hf_dataset_bucket_id(*, base_runtime: Any) -> str:
...
def _pointer_step(pointer: object, *, base_runtime: Any) -> int:
    if pointer is None:
        return 0
...
def next_unconsumed_block(
    *,
    ...
    remote_step = _pointer_step(pointer, base_runtime=base_runtime)
...
def stage_for_h100(
    *,
    ...
    run_root: Path,
    base_runtime: Any,
    resolve_presets: Callable[[str, str], tuple[Any, Any]],
    canonical_run_id: Callable[[Any, Any], str],
    provider_name: str,
) -> dict[str, object]:
    ...
    cursor = next_unconsumed_block(
        training_run_id=training_run_id,
        run_root=run_root,
        base_runtime=base_runtime,
    )
    bucket_id = hf_dataset_bucket_id(base_runtime=base_runtime)
    ...
    print(
        json.dumps({f"{provider_name}_cpu_dataset_stage": result}, sort_keys=True),
        flush=True,
    )
    return result
...
def _preauthorize_existing_runtime_verification(
    *,
    ...
    """Re-hash CPU staging and write the marker consumed by the shared runtime."""
...
def _install_incremental_plan_adapter(
    dataset: Path,
    *,
    profile_key: str,
    base_runtime: Any,
    ...
    """Make the shared runtime use the frozen pre-production trainer horizon."""
    ...
    def derive_incremental(
        ...
        if requested_profile != profile_key:
            raise RuntimeError(
                f"incremental dataset profile changed during {provider_name} planning"
            )
        plan: dict[str, Any] = {
            "version": 1,
...
def run_staged_training(
    *,
    ...
    """Run the shared trainer against the CPU-staged rolling dataset."""
    ...
    if not dataset.is_absolute():
        raise RuntimeError(
            f"rolling {provider_name} dataset directory must be an absolute CPU-staged path"
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"rolling {provider_name} dataset must live inside the cache Volume"
        ) from error
    ...
    if profile.incremental_frontier:
        _install_incremental_plan_adapter(
            dataset_resolved,
            profile_key=token_preset.dataset_profile,
            base_runtime=base_runtime,
