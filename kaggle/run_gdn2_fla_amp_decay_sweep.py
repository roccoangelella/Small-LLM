#!/usr/bin/env python3
"""Locate the FLA GDN-2 AMP backward failure boundary without launching training.

Runs the complete Small-LLM GDN-2 layer with FP32 parameters + CUDA FP16
autocast, matching the trainer.  For each forced constant log-decay value it
compares FLA against the adaptive reference and reports whether every gradient
is finite and numerically close.

Run:
    python kaggle/run_gdn2_fla_amp_decay_sweep.py
"""
...
def force_constant_log_decay(torch, layer, value: float) -> None:
    """Choose parameters that make g exactly ``value`` for every position/channel."""
    magnitude = abs(float(value))
    with torch.no_grad():
        layer.decay_proj[0].weight.zero_()
        layer.decay_proj[1].weight.zero_()
        layer.A_log.fill_(math.log(magnitude))
        # softplus(dt_bias)=1, so g=-exp(A_log)*1=-magnitude.
...
def main() -> int:
    ...
    ensure_fla()
    print("=" * 78)
    print("Small-LLM FLA GDN-2 trainer-AMP decay sweep")
    print("=" * 78)
    ...
    print("[purpose] find the first constant log-decay where FLA backward becomes non-finite/mismatched")
    ...
    if failing:
        first = failing[0]
        print(f"first failing tested point: g={first} (64-token cumulative magnitude {abs(first)*64:.1f})")
    else:
        print("No failure found in the tested range through g=-6.")
    print("Next: compare this boundary with decay telemetry from the real step-4000 checkpoint before any training resume.")
    return 0
