#!/usr/bin/env python3
"""Fail-closed 1B profile overlay for the proven 20M/100M Kaggle launcher.
...
distinct finite dataset identity, W&B identity, output namespace, and 1B
manifest envelope.

The 1B experiment is a fresh seed-17 trajectory. It starts directly at the
already-qualified microbatch 4 and, on CUDA, uses the T4-qualified mixed FLA
GDN-2 backend from update 1. It is not initialized from a 500M checkpoint.
"""
...
BASE_SPEC = importlib.util.spec_from_file_location(
    "small_llm_run_20m_1b_base", BASE_IMPLEMENTATION
)
...
DEFAULT_COMMIT = "__PIN_20M_1B_LAUNCH_COMMIT__"
DATASET_RUN_ID = "20m-1b-dataset-001"
...
for _name, _expected in _EXPECTED_BASE.items():
    if getattr(base, _name, None) != _expected:
        raise RuntimeError(
            f"1B launcher base contract changed: {_name}="
...
def arguments(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--launch-commit",
        default=os.environ.get("SMALL_LLM_1B_LAUNCH_COMMIT", DEFAULT_COMMIT),
    )
...
def find_dataset(explicit: Path | None) -> tuple[Path, list[dict[str, Any]]]:
    ...
    if len(matches) != 1:
        raise base.LaunchFailure(
            f"Expected exactly one attached 1B dataset; found {len(matches)}.\n"
...
def trainer_command(
    ...
    *,
    ...
    command = ["1b-tokens" if item == "100m-tokens" else item for item in command]
    return command
...
def profile_print(*values: object, **kwargs: object) -> None:
    replaced = [
        value.replace("100M-token run completed", "1B-token run completed")
