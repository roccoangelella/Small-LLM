#!/usr/bin/env python3
"""Build, verify, and privately publish the fixed 1B-token Kaggle dataset.
...
1B production identity and environment namespace.
"""
...
SPEC = importlib.util.spec_from_file_location("small_llm_build_and_push_1b_base", IMPLEMENTATION)
...
SLUG = "small-llm-20m-1b-dataset-001"
...
DEFAULT_DATASET = "/data/small-llm/20m-1b-dataset-001"
DEFAULT_OPS = "/data/small-llm/20m-1b-ops"
HANDLE_ENV = "SMALL_LLM_1B_KAGGLE_DATASET_HANDLE"
...
for _name, _expected in _EXPECTED_BASE.items():
    if getattr(base, _name, None) != _expected:
        raise RuntimeError(
            f"1B publisher base contract changed: {_name}="
...
def derive_plan(root: Path, prefix: str, config: base.Config) -> None:
    base.run(
        [
            sys.executable,
            "-m",
            "dataset.qualification_1b_report",
            "--dataset-dir",
            str(root),
            "--drive-manifest",
...
def _install_environment_aliases() -> None:
    ...
    aliases = {
        "SMALL_LLM_100M_WEIGHTS_FILE": "SMALL_LLM_1B_WEIGHTS_FILE",
        "SMALL_LLM_100M_DATASET_DIR": "SMALL_LLM_1B_DATASET_DIR",
        "SMALL_LLM_100M_OPS_DIR": "SMALL_LLM_1B_OPS_DIR",
    }
