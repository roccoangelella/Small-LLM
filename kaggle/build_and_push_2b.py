#!/usr/bin/env python3
"""Build, verify, and privately publish the fixed 2B-token Kaggle dataset.
...
2B production identity and environment namespace.
"""
...
SPEC = importlib.util.spec_from_file_location("small_llm_build_and_push_2b_base", IMPLEMENTATION)
...
SLUG = "small-llm-20m-2b-dataset-001"
...
for _name, _expected in _EXPECTED_BASE.items():
    if getattr(base, _name, None) != _expected:
        raise RuntimeError(
            f"2B publisher base contract changed: {_name}="
...
def _install_environment_aliases() -> None:
    ...
    aliases = {
        "SMALL_LLM_100M_WEIGHTS_FILE": "SMALL_LLM_2B_WEIGHTS_FILE",
        "SMALL_LLM_100M_DATASET_DIR": "SMALL_LLM_2B_DATASET_DIR",
        "SMALL_LLM_100M_OPS_DIR": "SMALL_LLM_2B_OPS_DIR",
    }
