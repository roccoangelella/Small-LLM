"""Offline tests for the fixed 20M-model/2B-token Kaggle profile overlay."""
...
import run_20m_2b_data_scaling as launcher  # noqa: E402
...
class Kaggle20M2BProfileTests(unittest.TestCase):
    def test_profile_does_not_mutate_100m_module(self) -> None:
        ...
        self.assertEqual(launcher.DATASET_RUN_ID, "20m-2b-dataset-001")
        self.assertEqual(launcher.PROFILE, "20m-2b-data-scaling-v1")

    def test_2b_manifest_matches_and_1b_manifest_is_rejected(self) -> None:
    ...
    def test_trainer_command_uses_distinct_2b_telemetry_identity(self) -> None:
        ...
        self.assertEqual(
            ...
            "20M model on 2B tokens",
        )
        tags = command[command.index("--wandb-tags") + 1 :]
        self.assertIn("2b-tokens", tags)
        self.assertNotIn("100m-tokens", tags)

    def test_fresh_2b_run_skips_probes_and_selects_microbatch_four(self) -> None:
