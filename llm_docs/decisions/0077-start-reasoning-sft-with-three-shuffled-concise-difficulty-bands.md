...
# ADR 0077 — Start reasoning SFT with three shuffled concise difficulty bands

## Context and problem statement

The approximately-100M student needs an initial reasoning curriculum that fits its capacity without assuming that long traces or a monotonic easy-to-hard order are beneficial.

## Considered options

1. Train long monologue-style traces from the outset.
2. Present a strict easy-to-hard curriculum.
3. Mix concise, verifiable examples from three explicit difficulty bands.
...
- The first reasoning-SFT corpus must expose three explicit difficulty bands and mix them globally during training.
- Initial traces remain concise and capacity-aligned; long reasoning trajectories require a later decision.
- Dataset source, teacher, verifier, budget, and exact band definitions remain separate gates.
