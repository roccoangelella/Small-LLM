...
# ADR 0082 — Focus R0 on logic primitives and defer exact computation

## Context and problem statement

The approximately-100M student cannot efficiently internalize every reasoning and exact-computation skill in its first reasoning stage, so R0 needs a narrow transferable capability boundary.

## Considered options

1. Center R0 on exact arithmetic, algebra, and calculus.
2. Mix unrestricted reasoning and computation tasks.
3. Focus R0 on logic and magnitude primitives, then teach exact computation through later tool use.
...
- R0 generation and verification focus on the seven accepted logic/magnitude families across L1–L3.
- Exact arithmetic, algebra, calculus, and long computation traces remain outside the R0 promotion gate.
- Later tool-use work must separately teach operation selection, invocation, and result interpretation.
