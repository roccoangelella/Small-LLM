...
## Active scaling trajectory — stronger-decay step-15,500 continuation through 10B

ADR 0094 supersedes ADR 0093, ADR 0092, and the original long flat-`3e-4` WSD trajectory. The main continuation must fork the exact uncooled `100m-10b-data-001/checkpoints/step-00015500` state and use `beam/strong_decay_10b_from_15500.py` under the separate run ID `100m-10b-strong-decay-from-step15500`.
...
The ADR-0093 trajectory showed its best fixed-prefix validation behavior near the end of the fast settle, followed by a rise after the transition to its gentler long phase. ADR 0094 therefore lowers the fast-settle endpoint from `1.5e-4` to `1.0e-4`. The long inverse-square-root phase is anchored at that lower rate, making every pre-terminal-cooldown LR one-third lower than the corresponding ADR-0093 value while retaining the near-inverse-square-root continuation shape supported by recent WSqD/power-schedule work.

The launcher must fail closed if the exact original step-15,500 source is unavailable, CPU-stage and verify the checkpoint-aligned dataset window before GPU allocation, and keep local/W&B/HF checkpoint namespaces separate from the original run, the cooldown probe, and the superseded ADR-0093 continuation.

## Active diagnostics — earlier post-15,500 branches

ADR 0091's `100m-10b-decay-probe-step15500` branch remains useful schedule evidence. The ADR-0093 `100m-10b-aggressive-wsqd-from-step15500` branch is also evidence: its validation loss fell during the fast settle and then rose after the transition to the gentler long phase, motivating ADR 0094.

Do not promote either diagnostic/older continuation checkpoint into the new 10B run and do not reheat a cooled model. The active continuation always starts from the original uncooled step-15,500 checkpoint.
...
The original 76,294-update / 10,000,007,168-target ADR-0057 WSD contract remains historical/reproducible, but it is no longer authorized as the main continuation schedule under ADR 0094.
...
- Does the stronger-decay 10B continuation keep validation loss falling after step 17,789 rather than reproducing ADR 0093's post-transition rise?
...
- New finite scaling trajectories start from fresh initialization unless a later ADR says otherwise; ADRs 0091 and 0094 are explicit continuation/diagnostic exceptions.
