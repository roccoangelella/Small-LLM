...
MoE 100B SuperBPE dataset production is active on `moe-8e-top1` at commit `e2e75b1`: Modal app `ap-h4isK62Lffpz579Ea4lZ5B`, launched 2026-09-11 11:01 UTC from tmux session `moe-100b-superbpe-public`. It targets `moe-100b-superbpe-b64-dataset-001` in the dedicated public HF Storage Bucket `roccoangelella/small-llm-moe-100b-superbpe-dataset`; the immutable run contract is public and source resolution is 100 files / 1,851.4 GiB. This is dataset production only and does not authorize 50B/100B-token model pretraining. Completion remains gated on the public-visibility, READY-frontier, coverage, manifest, and hash checks in ADR 0178.
