...
- ADR 0158 freezes `100b-b64-dataset-001`: the existing context-2048/block-64/approximately-1-GiB schema-v2 producer prebuilds the full 100B corpus in a dedicated public HF bucket. Modal and Beam are consumers only and require a completed READY frontier before GPU dispatch.
- ADRs 0160-0165 select the approximately-200M/100B trajectory and freeze its widened 20-layer hybrid geometry, inherited hybrid optimizer, exact WSqD anchors/timing, block-64 update contract, and Modal H100 / Beam RTX4090 provider boundary.
- ADR 0166 gives Modal and Beam one shared scientific profile registry plus one rolling-dataset staging/producer implementation under `providers/`. Stable provider paths remain thin bindings, both launchers accept compact `--profile MODEL-TOKENS`, and provider-specific runtimes remain separate.
- ADR 0167 records the local repair: `uv sync --locked` covers the default runtime/test groups, Kaggle root/`src` wrappers are import-safe and path-correct, R-SFT saved-plan drift is rejected, and historical nonconforming ADRs have an explicit SHA-256 baseline.
- Offline repair verification: 702 pytest passes/1 opt-in live-remote skip; 591 unittest tests, OK with the same skip. See `llm_docs/evidence/repository-repair-verification-2026-09-08.md`. Cloud qualification and external corpus verification remain outstanding.
