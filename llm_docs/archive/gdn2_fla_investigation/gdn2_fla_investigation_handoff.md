...
purpose: completed investigation handoff
---

# GDN-2 / FLA investigation handoff — final August 8 state

This is the consolidated historical handoff for the 20M/500M GDN-2 backend investigation. For current production behavior, read [`../../reference/gdn2_fla_backend.md`](../../reference/gdn2_fla_backend.md), current status, and the accepted ADRs.

## 1. Active experiment state at the end of the investigation
...
The checkpoint was clean. No qualification diagnostic executed an optimizer step, acknowledged block 4000, advanced the scheduler/data cursor, wrote W&B state, or published a checkpoint. **No FLA update 4001 had yet been accepted at this point in the investigation.**

## 2. Production decision at the time

The trajectory was qualified to resume from `step-00004000` using the mixed-precision FLA GDN-2 execution path on:
...
Checkpoint/model semantics remained unchanged:
...
Full-FP32 FLA also passed the final qualification but was slower. Mixed FLA was selected because it passed the same corrected synthetic and real-checkpoint gates and was fastest in the warmed benchmark.
...
## 4. GDN-2 semantic boundary that remained fixed
...
The oracle wraps only the adaptive recurrence in:
...
The corrected synthetic harness also fixed:
...
This means the full-FP32 mode passed, but the experiment no longer supported claiming that FP32 fixed a mixed-precision FLA bug because mixed FLA itself passed the corrected gate.
...
Mixed FLA was therefore the fastest tested backend that satisfied the exact-semantics correctness gate.
...
The implementation commit containing the qualified runtime was:

```text
c0214d00047c61a290d9a138a6bd94ed5701337c
```

The 500M launcher was pinned to that implementation commit. The later documentation/authorization commit intentionally pointed to the already-complete implementation commit so the launcher pin was not self-referential.

## 15. Scientific conclusions at investigation close

Strongly supported:

1. The adaptive backend was the main cause of the historical decay-related throughput pathology.
2. The original FLA AMP dtype mismatch was a genuine integration bug and was fixed by adapter dtype canonicalization.
...
4. With a deterministic finite FP32 oracle, `fla-core==0.5.2` mixed and full-FP32 FLA passed the entire requested synthetic decay sweep on T4.
5. Both modes passed the true step-4000/full-next-block forward and all-gradient parity gate with checkpoint loss scaling.
6. Warmed mixed FLA was approximately 11.59x faster than the adaptive reference on the real next block and faster than full-FP32 FLA.
7. The mixed `fla-core==0.5.2` path was qualified for production continuation of the active checkpoint.

Important boundaries at the time:

1. `fla-core==0.5.1` was not rerun through the final corrected full gate; production qualification was specifically for 0.5.2.
2. Qualification did not execute update 4001 or prove the remainder of the 500M trajectory would never encounter a future unrelated failure.
3. The fail-closed trainer, scaler, checkpointing, and validation protections remained necessary.
4. No model-semantic change had been authorized or implemented.

## 16. Production boundary and next action at the time
...
The next operational action at that point was the ordinary fail-closed 500M resume. The 500M trajectory later completed; current state is recorded elsewhere.

## 17. Durable evidence

Final evidence remains under `llm_docs/evidence/`, including:

```text
gdn2_fla_corrected_oracle_and_step4000_qualification_2026-08-08.md
gdn2_fla_fp32_qualification_corrected_2026-08-08.json
gdn2_fla_step4000_parity_2026-08-08.json
gdn2_fla_step4000_benchmark_2026-08-08.json
```

Historical failed evidence is also retained, including the old v0.5.1/v0.5.2 failed sweeps and the genuine first AMP dtype compile failure.
