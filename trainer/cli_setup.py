...
def setup(args: object):
    ...
    model_config = factory(architecture=args.architecture)
    model = SmallLLM(model_config)
    initialize_model(model, args.initialization)
    trainer_config = TrainerConfig(microbatch_size=args.microbatch_size,
        learning_rate=args.learning_rate, weight_decay=args.weight_decay,
        max_grad_norm=args.max_grad_norm, precision=args.precision, schedule=args.schedule,
        warmup_tokens=args.warmup_tokens, stable_tokens=args.stable_tokens,
        decay_tokens=args.decay_tokens, minimum_lr_ratio=args.minimum_lr_ratio,
        checkpoint_every_steps=args.checkpoint_every_steps,
        evaluation_every_steps=args.evaluation_every_steps, seed=args.seed)
    source = SchemaV2ShardReader(args.dataset_dir, split="train",
    ...
    if checkpoint_root is not None and (checkpoint_root / "checkpoint.json").is_file():
        ...
        identities = checkpoint_identity(args.dataset_dir, model_config=model_config,
            trainer_config=trainer_config, manifest_path=args.dataset_manifest,
            context_length=model_config.max_seq_len,
            sequences_per_block=args.sequences_per_block)
    coordinator = CheckpointCoordinator(args.checkpoint_dir,
        configuration_hash=identities[0], source_hash=identities[1], schema_hash=identities[2])
    if args.resume:
...
def validation_reader(args: object, model_config: object) -> SchemaV2ShardReader:
    return SchemaV2ShardReader(args.dataset_dir, split="validation",
        sequences_per_block=args.sequences_per_block,
        semantic_vocab_size=model_config.semantic_vocab_size,
        manifest_path=args.dataset_manifest,
        context_length=model_config.max_seq_len if args.dataset_manifest is not None else None)
