...
optimizer clock starts at zero, warms up briefly, may hold peak LR for an
explicit bounded span, settles, follows a long calibrated power law, and
finishes with a short linear cooldown.
"""
...
class FreshAggressiveDecayPlan:
    ...
    def schedule_anchor_tokens(self) -> int:
        return self.warmup_tokens + self.peak_tokens

    @property
    def settle_end_tokens(self) -> int:
        return self.schedule_anchor_tokens + self.settle_tokens

    def trainer_kwargs(self) -> dict[str, object]:
        return {
            "schedule": "wsqd",
            "warmup_tokens": self.warmup_tokens,
            "stable_tokens": self.peak_tokens,
...
def fresh_aggressive_decay_plan(
    ...
    *,
    ...
    required_fractions = (warmup_fraction, settle_fraction, cooldown_fraction)
    if any(
        not math.isfinite(float(value)) or not 0 < float(value) < 1
        for value in required_fractions
    ):
        raise ValueError("fresh decay warmup, settle, and cooldown fractions must lie in (0, 1)")
    if not math.isfinite(float(peak_fraction)) or not 0 <= float(peak_fraction) < 1:
        raise ValueError("fresh decay peak fraction must lie in [0, 1)")
    if sum(float(value) for value in required_fractions) + float(peak_fraction) >= 1:
    ...
    peak_tokens = (
        max(1, math.floor(total_tokens * peak_fraction))
        if float(peak_fraction) > 0
        else 0
    )
    ...
    if settle_end_tokens >= cooldown_start_tokens:
        raise ValueError("fresh decay horizon is too short for peak hold, settle, and power-law phases")
