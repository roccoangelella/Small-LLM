...
def _base_items() -> list[tuple[str, str, str, str]]:
    ...
    for family, rows in families.items():
        if len(rows) != 20:
            raise RuntimeError(
                f"Base Prompt v2 family {family!r} must contain exactly 20 cases"
            )
...
def _validate_base_prompt_cases(cases: Sequence[BasePromptCase]) -> None:
    if len(cases) != 120:
        raise RuntimeError(
            f"Base Prompt v2 must contain exactly 120 cases, got {len(cases)}"
        )
    ...
    if len(scored) != 100 or len(qualitative) != 20:
        raise RuntimeError(
            "Base Prompt v2 must contain 100 objective and 20 qualitative cases"
        )
    expected_families = {
        "factual",
        "arithmetic",
    ...
    for family in expected_families:
        ...
        if count != 20:
            raise RuntimeError(
                f"Base Prompt v2 objective family {family!r} must contain 20 cases, got {count}"
            )
    if any(case.family not in expected_families for case in scored):
        raise RuntimeError("Base Prompt v2 contains an unexpected objective family")
    if any(not isinstance(case.answer, str) or not case.answer for case in scored):
        raise RuntimeError("every objective Base Prompt v2 case needs a reference answer")


def base_prompt_cases_v2() -> tuple[BasePromptCase, ...]:
    scored = [
        BasePromptCase(name, family, prompt, 48, answer)
...
def _run_prompt_view(
    ...
    *,
    ...
    cases = BASE_PROMPT_CASES_V2 if suite == "full" else BASE_PROMPT_CASES_V2[:30]
    prompt_ids = [enc.encode(case.prompt, disallowed_special=()) for case in cases]
    requests = [
        GenerationRequest(
            tuple(ids),
            min(case.max_new_tokens, model_max_seq_len - len(ids)),
            seed + index * 1000,
        )
    ...
    generated_rows = sample_token_ids_batched(
        ...
        top_k=top_k,
        precision=precision,
        batch_size=BASE_PROMPT_BATCH_SIZE,
        progress_label=f"base-prompts-v2/{view}",
    )

    rows: list[dict[str, object]] = []
    for case, request, generated in zip(
        cases, requests, generated_rows, strict=True
    ):
        terminated = bool(generated and generated[-1] == enc.eot_token)
        ...
        rows.append(
            {
                ...
                "scored": not case.qualitative,
                "judge_status": "not_applicable" if case.qualitative else "pending",
                "prompt": case.prompt,
                "reference_answer": case.answer,
                "seed": request.seed,
                "generated_token_ids": generated,
                "continuation": text,
                "response_tokens": len(body),
...
def _summary(rows: Sequence[Mapping[str, object]]) -> dict[str, object]:
    scored = [row for row in rows if bool(row.get("scored"))]
    by_family: dict[str, object] = {}
    for family in sorted({str(row["family"]) for row in scored}):
        selected = [row for row in scored if row["family"] == family]
        by_family[family] = {"cases": len(selected), "judge_status": "pending"}
    return {
        "cases": len(rows),
        "scored_cases": len(scored),
        "qualitative_cases": len(
            [row for row in rows if bool(row.get("qualitative"))]
        ),
        "judge_status": "pending" if scored else "not_applicable",
        "accuracy": None,
        "by_family": by_family,
        "mean_response_tokens": (
            mean(float(row["response_tokens"]) for row in rows)
            if rows
            else math.nan
        ),
    }
...
def run_base_prompt_suite_v2(
    ...
    *,
    ...
    """Generate raw Base Prompt v2 evidence without local answer matching.

    Objective cases are intentionally left unjudged here.  The durable model
    output can be scored later by the GemRouter semantic judge without rerunning
    the checkpoint or recompiling GPU kernels.
    """

    greedy = _run_prompt_view(
        model,
    ...
    return {
        ...
        "suite_identity": {
            ...
            "full_scored_cases": 100,
            "full_qualitative_cases": 20,
            "full_unique_prompts": 120,
            "budget": "native per prompt",
        },
        "scoring_contract": {
            "scoring_id": BASE_PROMPT_SCORING_ID,
            "method": "external GemRouter semantic judge",
            "local_string_or_regex_scoring": False,
            "status": "pending",
            "objective_cases": "judge prompt + reference_answer + continuation semantically",
            "qualitative_cases": "unscored readable evidence",
        },
        "execution": {
            "length_bucketed": True,
            "max_batch_size": BASE_PROMPT_BATCH_SIZE,
        },
        "greedy": {
            "sampling": {
                "temperature": 0.0,
...
class SmallLLMHarnessLM(_HarnessBase):
    def __init__(
        ...
        *,
        max_seq_len: int,
        precision: str,
        batch_size: int = L20_MAX_BATCH_SIZE,
        max_batch_tokens: int = L20_MAX_BATCH_TOKENS,
    ) -> None:
        try:
            super().__init__()
        except TypeError:
    ...
    def tok_encode(
        ...
        **kwargs,
        ...
        return list(self.encoding.encode(string, disallowed_special=()))

    def _encode_pair(self, context: str, continuation: str):
        if not context:
            return [self.prefix_token_id], self.tok_encode(continuation)
        whole = self.tok_encode(context + continuation)
        ctx = self.tok_encode(context)
        return ctx, whole[len(ctx) :]

    def _prepare_pair(
        self, ctx: Sequence[int], cont: Sequence[int]
    ) -> tuple[list[int], int]:
    ...
    def _score_prepared_batch(
        self, prepared: Sequence[tuple[list[int], int]]
    ) -> list[tuple[float, bool]]:
        if not prepared:
            return []
        width = max(max(1, len(full) - 1) for full, _ in prepared)
        input_ids = torch.zeros(
            (len(prepared), width), dtype=torch.long, device=self.device
        )
        for row, (full, _) in enumerate(prepared):
            if len(full) > 1:
                input_ids[row, : len(full) - 1] = torch.tensor(
                    full[:-1], dtype=torch.long, device=self.device
                )
        ...
        for row, (full, label_start) in enumerate(prepared):
            ...
            results.append(
                (
                    float(
                        log_probs.gather(1, selected_labels[:, None]).sum()
                    ),
                    bool(
                        torch.eq(
    ...
    def loglikelihood(self, requests: list[Any], disable_tqdm: bool = False):
        ...
        for index, request in enumerate(requests):
            ctx, cont = self._encode_pair(
                str(request.args[0]), str(request.args[1])
            )
            prepared = self._prepare_pair(ctx, cont)
            encoded.append(
                (index, prepared, max(1, len(prepared[0]) - 1))
            )
        encoded.sort(key=lambda item: (item[2], item[0]))

        batches: list[list[tuple[int, tuple[list[int], int], int]]] = []
        current: list[tuple[int, tuple[list[int], int], int]] = []
        current_width = 0
        for item in encoded:
            candidate_width = max(current_width, item[2])
            candidate_count = len(current) + 1
            if current and (
                candidate_count > self.batch_size
                or candidate_width * candidate_count > self.max_batch_tokens
            ):
                batches.append(current)
                current = []
                current_width = 0
                candidate_width = item[2]
            current.append(item)
            current_width = max(current_width, candidate_width)
        if current:
            batches.append(current)
        ...
        next_report = 10
        print(
            f"[L20 likelihood] scoring {total} requests in {len(batches)} "
            f"length-bucketed batches (max_batch_size={self.batch_size}, "
            f"max_batch_tokens={self.max_batch_tokens})",
            flush=True,
        )
        for batch in batches:
            scores = self._score_prepared_batch([item[1] for item in batch])
            for (original_index, _, _), score in zip(
            ...
            if percent >= next_report or completed == total:
                print(
                    f"[L20 likelihood] {completed}/{total} requests complete "
                    f"({percent}%)",
                    flush=True,
                )
        ...
        if any(result is None for result in results):
            raise RuntimeError(
                "L20 batched scorer failed to populate every request"
            )
    ...
    def loglikelihood_rolling(
        ...
        raise RuntimeError("rolling likelihood disabled for L20")

    def generate_until(
        self, requests: list[Any], disable_tqdm: bool = False
    ):
        raise RuntimeError("generation disabled for L20")

    def apply_chat_template(self, chat_history, add_generation_prompt=True):
        raise RuntimeError("chat templates disabled for L20")
...
def run_l20_conditional_likelihood(
    model: nn.Module,
    *,
    model_max_seq_len: int,
    precision: str,
    ...
    adapter = SmallLLMHarnessLM(
        model, max_seq_len=model_max_seq_len, precision=precision
    )
    result = lm_eval.simple_evaluate(
        model=adapter,
        tasks=list(L20_TASKS),
        num_fewshot=0,
        batch_size=L20_MAX_BATCH_SIZE,
        limit=100 if suite == "fast" else None,
        bootstrap_iters=0,
        log_samples=False,
        random_seed=17,
        numpy_random_seed=17,
        torch_random_seed=17,
        fewshot_random_seed=17,
    )
    raw = result.get("results", {}) if isinstance(result, Mapping) else {}
    rows: dict[str, object] = {}
    values: list[float] = []
    for task in L20_TASKS:
        row = raw.get(task, {}) if isinstance(raw, Mapping) else {}
        metric = _metric(row) if isinstance(row, Mapping) else None
    ...
    return {
        "schema": "small-llm-l20-conditional-likelihood-v1",
        "harness": "lm-evaluation-harness==0.4.12",
        "tasks": list(L20_TASKS),
        "limit_per_task": 100 if suite == "fast" else None,
        "mean_6": mean(values) if len(values) == 6 else math.nan,
        "task_results": rows,
        "request_policy": {
            "loglikelihood": True,
            "rolling_loglikelihood": False,
            "generation": False,
        },
        "execution": {
            "length_bucketed": True,
            "max_batch_size": L20_MAX_BATCH_SIZE,
            "max_batch_tokens": L20_MAX_BATCH_TOKENS,
