...
class TrainingSession:
    ...
    def step(self, timeout: float | None = None) -> StepMetrics:
        ...
        data_wait = max(time.perf_counter() - wait_started, 0.0)
        metrics = self.engine.train_batch(batch)
        self.source.acknowledge(batch.block_id)
        return replace(metrics, data_wait_seconds=data_wait)

    def save_checkpoint(
        self,
        coordinator: CheckpointCoordinatorLike,
        checkpoint_id: str,
        *,
    ...
    def load_checkpoint(
        self, coordinator: CheckpointCoordinatorLike, checkpoint_id: str
    ) -> dict[str, object]:
